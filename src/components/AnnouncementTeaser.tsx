        ...
        <div className="glass-panel rounded-[30px] border border-gold/16 px-5 py-5 md:px-8 md:py-7">
          <div className="flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
            <div className="max-w-2xl">
              <p className="section-kicker mb-4">{isTr ? "Rehberler" : "Guides"}</p>
              <h2 className="font-display text-[1.9rem] font-bold leading-tight tracking-[-0.03em] text-white md:text-[2.4rem]">
                {isTr ? "Yolculuk planınızı rehberlerle netleştirin" : "Plan your ride with practical guides"}
              </h2>
              <p className="mt-3 max-w-xl text-sm leading-7 text-text/78 md:text-base md:leading-8">
                {isTr
                  ? "Havalimanı, otogar, gece ulaşımı, kısa mesafe ve özel ihtiyaçlar için hazırladığımız kalıcı rehberleri tek yerde topladık."
                  : "We gathered evergreen guides for airport trips, bus terminal rides, night transport, short distances, and special travel needs."}
              </p>
            </div>
            ...
            <div className="flex flex-col gap-3 md:items-end">
              <div className="rounded-full border border-gold/20 bg-white/[0.03] px-3 py-1 text-[0.72rem] uppercase tracking-[0.24em] text-gold/80">
                {isTr ? "İçerik merkezi" : "Content hub"}
              </div>
